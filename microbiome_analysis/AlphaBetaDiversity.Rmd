...
## Read in data tables for phyloseq analysis and remove low abundance taxa
...
# create single dataframe for plotting because samples are ordered differently in the pcx dataframe than in the sample metadata
...
single_df$Treatment<-factor(single_df$Treatment,levels=c("fully closed","partially closed","open"))
single_df$Timepoint<-factor(single_df$Timepoint,levels=c("before","during","after"))

# not using for publication - both coral species in one plot
pdf("PCA_96.pdf",bg ="white",width = 8.5)
p<-ggplot(single_df,aes(x=PC1,y=PC2,color=single_df$Treatment,shape=single_df$Species))
p<-p+geom_point(size=2)+
  geom_text(label = single_df$sample, nudge_x = 2, nudge_y = 3, color = "black", alpha = 0.6)+ #adds the sample name to points
  ...
  facet_grid(~single_df$Timepoint)
p + coord_fixed()
dev.off()

# using for publication - facet by both timepoint and coral species
pdf("PCA_96_facet.pdf",bg ="white",width = 8.5)
p<-ggplot(single_df,aes(x=PC1,y=PC2,color=single_df$Treatment))
p<-p+geom_point(size=2)+
  theme(axis.title = element_text(size=14))+
  theme(axis.text=element_text(size=12))+
  theme(legend.title = element_text(size=14))+
  ...
  scale_color_manual(values=cols)+
  guides(fill = guide_legend(override.aes=list(shape=21)))+
  facet_grid(single_df$Species~single_df$Timepoint)
